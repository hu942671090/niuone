...
if /I "%~1"=="--admin-password" (
    if "%~2"=="" (
        echo --admin-password requires a value 1>&2
        call :show_usage 1>&2
        exit /b 2
    )
...
if /I "%ARG:~0,17%"=="--admin-password=" (
    set "ADMIN_PASSWORD=%ARG:~17%"
    set "ADMIN_PASSWORD_SET=1"
    shift
    goto parse_args
)
...
if "%ADMIN_PASSWORD_SET%"=="1" (
    call :save_env_value DASHBOARD_ADMIN_PASSWORD "%ADMIN_PASSWORD%"
    if errorlevel 1 exit /b 1
...
echo   --admin-password VALUE  Save the admin password to dashboard.env before starting
...
echo   DASHBOARD_AUTH_ENABLED Local auth switch, default on first run: 0
echo   DASHBOARD_ADMIN_PASSWORD
echo                         Admin password default on first run
exit /b 0
...
set "DEFAULT_AUTH_ENABLED=%DASHBOARD_AUTH_ENABLED%"
if not defined DEFAULT_AUTH_ENABLED set "DEFAULT_AUTH_ENABLED=0"
(
    ...
    echo DASHBOARD_AUTH_ENABLED=%DEFAULT_AUTH_ENABLED%
