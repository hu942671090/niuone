...
单页最多 100 只股票，并复用 Dashboard 限流和缓存。返回结果按股票代码去重，`sector` 提供所属行业，重复榜单记录保留在 `details` 中。问财响应属于研究数据快照，发生超时、计数不一致或上游失败时会返回明确状态，
不会覆盖账户、成交或其他真实交易记录。Dashboard 的 `/dragon-tiger` 栏目优先展示本地成功快照；Cron 默认在 A 股交易日北京时间
18:00 更新 `.local-data/runtime/cron/output/iwencai_dragon_tiger_latest.json`，空结果或失败不会覆盖上一份有效快照。
