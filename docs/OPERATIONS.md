...
Windows 使用 `run.bat --admin-password "change-this-to-a-strong-password"`。
...
需要保护设置页时，优先在设置页或启动参数中设置管理员密码，并妥善保存本地备用管理员凭据。
...
| 消息面预检 API | `DASHBOARD_NEWS_BASE_URL`、`DASHBOARD_NEWS_API_KEY`、`DASHBOARD_NEWS_MODEL`、`DASHBOARD_NEWS_MAX_TOKENS` |
...
`*_CONTEXT_LENGTH` 仅表示模型上下文窗口，留空表示使用模型/网关默认窗口；`*_MAX_TOKENS` 仅用于覆盖请求体里的 `max_tokens`，留空表示使用各场景内置输出预算。

买卖决策情报包默认开启。每次 B1 扫描后的模型决策都会读取盘面监控、隔夜美股、指数行情、板块涨跌、行业资金、热门股、候选消息面和账户仓位摘要，并把压缩后的 `decision_intelligence` 写入模拟交易决策日志。行情源失败时会保留 `source_status`，本轮决策继续按可用信息和既有风控执行。
