...
The iWencai source is disabled by default. Enable it and save the API key under **iWencai Data Source** in the admin settings, then open `/dragon-tiger` to browse dated top-five buy/sell institution, brokerage, and explicitly tagged hot-money/quant seats and amounts, or query dated research snapshots through `/api/iwencai/dragon-tiger`. Cron refreshes the latest snapshot at 18:00 China time on A-share trading days and archives it under `iwencai_dragon_tiger/YYYY-MM-DD.json`; empty or failed responses preserve the last valid data, and a same-day seat-detail failure does not overwrite archived seat rows. The key remains only in the private local `dashboard.env` and is never echoed by the page.
