...
The iWencai source is disabled by default. Enable it and save the API key under **iWencai Data Source** in the admin settings, then open `/dragon-tiger` or query dated research snapshots through `/api/iwencai/dragon-tiger`. Cron refreshes the durable snapshot at 18:00 China time on A-share trading days by default; empty or failed responses preserve the last valid data. The key remains only in the private local `dashboard.env` and is never echoed by the page.
