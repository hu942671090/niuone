...
`run.sh` 和 `run.ps1` 的本地首次运行默认关闭访问认证，只适合监听 `127.0.0.1` 的个人使用。如果要开放到局域网或公网，请先在设置页或 `.local-data/dashboard.env` 中开启访问认证并设置管理密码。
...
| `DASHBOARD_AUTH_ENABLED` | 一键本地启动默认 `0` | 访问认证开关，公网或多人访问必须开启 |
