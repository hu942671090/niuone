...
| A 股候选股消息面预检 | 具备实时搜索能力的模型 | `DASHBOARD_NEWS_BASE_URL`、`DASHBOARD_NEWS_API_KEY`、`DASHBOARD_NEWS_MODEL`、`DASHBOARD_NEWS_MAX_TOKENS` |
...
`*_CONTEXT_LENGTH` 只表示模型上下文窗口，留空表示使用模型/网关默认窗口；`*_MAX_TOKENS` 只用于覆盖本次请求的最大输出长度，留空表示使用各场景内置输出预算。
