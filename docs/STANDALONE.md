...
问财数据源默认关闭；在管理设置页的“问财数据源”中启用并保存密钥后，可在 `/dragon-tiger` 查看龙虎榜，也可通过 `/api/iwencai/dragon-tiger` 按日期读取研究快照。Cron 默认在 A 股交易日北京时间 18:00 更新本地快照；空结果或失败保留上一份有效数据。密钥只保存在本机私有 `dashboard.env`，页面不会回显。
