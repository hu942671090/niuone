...
| `app/dashboard/` | 看板请求解析、安全辅助、公开展示投影、版本化快照、API 缓存编排和榜单规则 | 交易决策、账户文件重建 |
...
Dashboard 保持原生 HTML、CSS、JavaScript 页面和单端口部署，同时把高频展示读取与服务端计算解耦：
...
- `dashboard/server.py` 在同一端口提供原页面、增量快照、兼容 API 和 `/admin`；
- `frontend/dashboard.js` 先检查轻量 latest 指针，只在区块摘要变化时加载对应数据，完整历史每个页面会话最多成功加载一次。
