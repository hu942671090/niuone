...
export DASHBOARD_TRADER_SCRIPT="${DASHBOARD_TRADER_SCRIPT:-$BASE/app/niuniu_practice_trader.py}"
mkdir -p "$DASHBOARD_HOME/cron/output" "$DASHBOARD_HOME/logs"
exec "$PYTHON_BIN" "$BASE/app/niuone_dashboard.py" --host "$DASHBOARD_HOST" --port "$DASHBOARD_PORT"
