"""Build and persist normalized dashboard market-report records."""
...
class HistoryStore(Protocol):
    ...
    def upsert_many(self, messages: list[dict[str, Any]]) -> int: ...


def to_cn_datetime(value: datetime | None = None) -> datetime:
    if value is None:
...
def store_market_report(
    ...
    *,
    ...
    guidance_extractor: Callable[[str], list[str]] = extract_decision_guidance,
) -> int:
    """Store one market report through an explicitly supplied history store."""
    body = (content or "").strip()
    if not body:
        return 0

    local_dt = to_cn_datetime(run_dt)
    env = os.environ if environ is None else environ
    run_key = env.get("NIUONE_CRON_RUN_KEY") or f"{job_id}:{local_dt.strftime('%Y-%m-%d_%H-%M-%S')}"
    source_id = f"cron_output_{job_id}"
    message = {
        "id": history_store.stable_id("market_monitor", job_id, run_key),
        ...
        "metadata": {
            "job_name": title,
            "decision_guidance": guidance_extractor(body),
            "run_key": run_key,
        },
    }
    count = history_store.upsert_many([message])
    if count != 1:
