...
MONITOR = Path(
    os.environ.get("X_WATCHLIST_MONITOR")
    or str(SCRIPT_DIR / "entrypoints" / "x_watchlist_monitor.py")
).expanduser()
...
def parse_env_file(path: Path = DASHBOARD_ENV_FILE) -> dict[str, str]:
    if not path.exists():
        return {}
    values = parse_env_text(path.read_text(encoding="utf-8", errors="ignore"))
...
def env_int(name: str, default: int, env: dict[str, str] | None = None) -> int:
    return _env_int(name, default, env if env is not None else os.environ)
